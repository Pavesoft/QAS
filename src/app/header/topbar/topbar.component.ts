...
export class TopbarComponent implements OnInit {
  ...
  onLogin() {
    if (this.loginForm.invalid) {
      this.errorMessage = 'Invalid input. Please correct the errors and try again.'; // Display an error message if invalid
    ...
    this.http.post('http://10.0.51.3:8091/users/login', loginData)
      .pipe(
        catchError((error) => {
