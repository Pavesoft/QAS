...
export class CheckoutDialogComponent {
  ...
  ngOnInit() {
    ...
    if (successIndicator === this.resultIndicator) {
      this.ecommService
        .setPaymentFlag(uid, successIndicator)
    ...
    if (window.location.host === origin) {
      if (successIndicator === this.resultIndicator) {
        ...
        console.log(
          "error : payment_not_completed, please contact support@quadrant-solutions.com with your details used when placing order."
        );
      }
    } else {
      console.log(
        "error : 203 Non-Authoritative Information OR origin_mismatch, please contact support@quadrant-solutions.com with your details used when placing order."
      );
    }
